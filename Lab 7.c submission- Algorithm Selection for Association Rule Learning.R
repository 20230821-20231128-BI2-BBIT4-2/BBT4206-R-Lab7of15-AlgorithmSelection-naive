...
## the dataset in use is that of groceries bought by customers

# Source: https://www.kaggle.com/datasets/heeraldedhia/groceries-dataset

# We can read data from an Excel csv as follows:
retail <- read_csv("data/Groceries_dataset.csv")
dim(retail)
...
retail_removed_vars <-
  retail %>% dplyr::select(-Member_number)
...
# Ensure the customer's item description is recorded as categorical data
retail_removed_vars_obs %>% mutate(itemDescription = as.factor(itemDescription))
...
retail_removed_vars_obs$trans_date <-
  as.Date(retail_removed_vars_obs$Date)
...
length(subset_rules)
association_rules_no_reps <- association_rules_prod_name[-subset_rules]
